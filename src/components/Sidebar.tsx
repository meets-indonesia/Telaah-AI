...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navItems = [
    { id: "overview", label: "Ringkasan 360°", icon: LayoutDashboard },
    { id: "harmonic", label: "Harmonic PRZ Engine", icon: Compass, badge: "NEW" },
    { id: "technical", label: "Terminal Teknikal", icon: CandleIcon },
    { id: "insider", label: "Whale & Insider Radar", icon: Radar },
    ...(isCommodity ? [{ id: "commodity", label: "Commodity Lens", icon: Pickaxe, badge: "Aktif" }] : []),
    { id: "all", label: "Semua Modul Lengkap", icon: FileSpreadsheet },
  ];

  return (
    <aside
      className={`fixed top-0 left-0 bottom-0 z-40 bg-[#070b13] border-r border-slate-800/80 transition-all duration-300 flex flex-col justify-between select-none ${
    ...
    >
      ...
      <div>
        <div className="p-3.5 border-b border-slate-800/80 flex items-center justify-between">
          ...
          {isCollapsed && (
            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white mx-auto shadow-md">
              <ShieldCheck className="w-5 h-5" />
            </div>
          )}
          ...
          <button
            ...
            className="p-1 rounded-lg bg-slate-800/60 hover:bg-slate-700 text-slate-400 hover:text-white transition"
