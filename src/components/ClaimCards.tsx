      ...
      <div className="flex items-center justify-between mb-4 pb-3 border-b border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Pemisahan klaim atomik dari teks masukan dan verifikasi silang terhadap data pasar & finansial.
          </p>
        </div>
      </div>
        ...
        {claims.map((claim, idx) => {
          ...
          return (
            ...
            >
              ...
              {/* Original Claim Text */}
              <blockquote className="text-sm font-medium text-slate-200 italic border-l-2 border-blue-500 pl-3 py-0.5 my-2">
                "{claim.originalText}"
              </blockquote>

              {/* Factual Metric / Counter Evidence */}
              {claim.factualMetricValue && (
                <div className="mt-2.5 mb-1.5 text-xs bg-slate-950/60 p-2.5 rounded-lg border border-slate-800/80 flex items-start gap-2">
                  <span className="text-blue-400 font-semibold shrink-0">Fakta Bursa:</span>
                  <span className="text-slate-300 font-mono">{claim.factualMetricValue}</span>
                </div>
              )}
              ...
              {claim.evidenceIds && claim.evidenceIds.length > 0 && (
                <div className="flex items-center gap-1.5 mt-3 pt-2 border-t border-slate-800/60 text-[11px] text-slate-500">
                  <span>Referensi Bukti:</span>
                  {claim.evidenceIds.map((evId) => (
                    <button
                      key={evId}
                      onClick={() => onSelectEvidence?.(evId)}
                      className="px-1.5 py-0.5 rounded bg-slate-800 text-blue-400 hover:text-blue-300 hover:bg-slate-700 font-mono transition"
                    >
                      #{evId}
                    </button>
                  ))}
