...
export const StockMiniCard: React.FC<StockMiniCardProps> = ({
  ...
  return (
    <div className="mt-2.5 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#10131d] overflow-hidden text-xs transition-colors">
      {/* Header Strip */}
      <div className="p-3 border-b border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-[#131622] flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className="px-1.5 py-0.5 rounded bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-slate-100 font-mono font-bold text-xs tracking-tight">
            {card.symbol}
          </span>
        ...
        <div className={`flex items-center gap-1 px-2 py-0.5 rounded border text-[10px] font-mono font-semibold ${style.badge}`}>
          {style.icon}
      ...
      <div className="p-3 space-y-2">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5">
          {card.highlights.map((h, i) => (
            <div
              key={i}
              className="p-2 rounded bg-slate-50 dark:bg-[#141722] border border-slate-100 dark:border-slate-800/60"
            >
              <div className="text-[10px] font-mono text-slate-400 uppercase tracking-wider">
                {h.title}
              </div>
              <div className="text-[11px] font-medium text-slate-800 dark:text-slate-200 mt-0.5 line-clamp-2">
                {h.desc}
              </div>
            </div>
          ))}
        ...
        {card.retailTakeaway && (
          <div className="p-2.5 rounded bg-slate-50 dark:bg-[#121520] border-l-2 border-l-indigo-500 border-y border-r border-slate-200 dark:border-slate-800 text-[11px] text-slate-700 dark:text-slate-300 leading-relaxed">
      ...
      {/* Actions Strip */}
      <div className="px-3 py-2 border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-[#121520] flex flex-wrap items-center justify-between gap-2">
          ...
          {onQuickFollowUp && (
            <>
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 rounded bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-slate-100 text-[10px] font-mono transition"
              >
