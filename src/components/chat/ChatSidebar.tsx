      ...
      >
        ...
        <div className="p-3.5 border-b border-slate-200 dark:border-slate-800/80 flex items-center gap-2">
          <button
            onClick={onNewChat}
            type="button"
            className="flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl bg-zinc-900 hover:bg-zinc-700 text-white text-xs font-semibold shadow-sm shadow-zinc-700/20 transition-all active:scale-[0.98]"
          >
        ...
        <div className="flex-1 overflow-y-auto px-3 py-3 space-y-4">
          ...
          <div>
            <div className="px-2 mb-1.5 flex items-center justify-between text-[11px] font-semibold text-slate-500 dark:text-slate-400">
              <span className="flex items-center gap-1">
                <Sparkles className="w-3 h-3 text-zinc-700" /> Alat bantu pemula
              </span>
            </div>
          ...
          <div>
            <div className="px-2 mb-1.5 flex items-center justify-between text-[11px] font-semibold text-slate-500 dark:text-slate-400">
              <span className="flex items-center gap-1">
                <Bookmark className="w-3 h-3 text-zinc-700" /> Saham populer
              </span>
            </div>
            <div className="grid grid-cols-2 gap-1.5">
              {POPULAR_TICKERS.map((item) => (
                <button
                  ...
                  className="flex flex-col items-start p-2 rounded-lg text-left bg-slate-50 hover:bg-zinc-50/70 dark:bg-slate-900/60 dark:hover:bg-zinc-950/40 border border-slate-200/80 dark:border-slate-800/70 hover:border-zinc-300 dark:hover:border-zinc-800 transition group"
                >
                  <span className="text-xs font-bold text-slate-800 dark:text-slate-200 group-hover:text-zinc-900 dark:group-hover:text-zinc-300">
                    {item.symbol}
                  </span>
                  ...
                  return (
                    <div
                      ...
                      className={`group flex items-center justify-between px-2.5 py-2 rounded-xl text-xs cursor-pointer transition-all ${
                        isActive
                          ? "bg-zinc-50 dark:bg-zinc-950/50 text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-800/80 font-medium"
